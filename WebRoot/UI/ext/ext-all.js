...
Ext.EventManager = function() {
	...
	function b(D) {
		if (!k) {
			...
			if (Ext.isIE && C.bindIE) {
				if(c.detachEvent){c.detachEvent("onreadystatechange",C);}else if(c.removeEventListener){c.removeEventListener("readystatechange",C,false);}
			}
	...
	function a() {
		...
		if (Ext.isIE) {
			if (!C()) {
				C.bindIE = true;
				if(c.attachEvent){c.attachEvent("onreadystatechange",C);}else if(c.addEventListener){c.addEventListener("readystatechange",C,false);}
			}
		} else {
			if (Ext.isOpera) {
...
Ext.tree.TreeEventModel.prototype = {
	...
	getNode : function(b) {
		...
		if (a = b.getTarget(".x-tree-node-el", 10)) {
			var c = Ext.fly(a, "_treeEvents").getAttribute("tree-node-id", "ext") || a.getAttribute('ext:tree-node-id');
			if (c) {
...
Ext.tree.TreeNodeUI = Ext.extend(Object, {
	...
	renderElements : function(e, l, k, m) {
		...
		var g = Ext.isBoolean(l.checked), b, c = this.getHref(l.href), d = [
				...
				e.attributes.title?' title="'+e.attributes.title+'"':'',
