...
public class FieldTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		try {
			...
			// 父类
			if(null!=leadId){
				TbField tbField=manager.getById(leadId);
				if(null!=tbField){
			...
			if(null!=valueCode){
				TbField tbField=manager.getByCode(valueCode);
				if(null!=tbField){
			...
			if(null==value && null!=valueDefault){
				TbField tbField=manager.getByCode(valueDefault);
				if(null!=tbField){
					value=tbField.getFieldId();
				}
			}
