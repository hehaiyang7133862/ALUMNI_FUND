...
public class FoundationAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		try {
			bean=(TbFoundType)getCommonBo().get(TbFoundType.class, new Long(id));
		} catch (Exception e) {	}
		...
		if(type!=null && !"".equals(type)){
			String hql="from TbFields a where a.tbField.code='FOUNDATION_TYPE' and a.code='"+type+"'";
		...
		if(beanType!=null){
			// 总数量
			int count = getCommonBo().getHQLNum(hqlCount+" and a.foundtypeFid='"+beanType.getFieldId()+"'");
			...
			if(beanTypeList!=null && !beanTypeList.isEmpty()){
				for(int i=0;i<beanTypeList.size();i++){
					...
					int objcount = getCommonBo().getHQLNum(hqlCount+" and a.foundtypeFid='"+obj.getFieldId()+"'");
	...
	public String typeJson() throws Exception {
		...
		try{
			...
			if(eleList!=null && !eleList.isEmpty()){
				...
				String type=request.getParameter("type");
				if(type!=null && !"".equals(type)){
					beanType = (TbFields)getCommonBo().get(TbFields.class, new Long(type));
				}
				if(beanType==null){
				...
				for(int i=0;i<eleList.size();i++){
					...
					String hqlCount = "select count(*) from TbFoundation a where a.foundType in ("+objIds+")";
					if(beanType!=null){
						...
						if(beanTypeList!=null && !beanTypeList.isEmpty()){
							for(int j=0;j<beanTypeList.size();j++){
								...
								int fieldcount = getCommonBo().getHQLNum(hqlCount+" and a.foundtypeFid='"+field.getFieldId()+"'");
	...
	private void getAllFoundTypes(List beanList,TbFoundType bean)throws Exception{
		String hql = "from TbFoundType a where a.parentId="+bean.getTypeId()+" order by a.numOrder";
	...
	public String list() throws Exception{
		...
		if(typeId!=null&&!"".equals(typeId)&&!"1".equals(typeId)){
			try {
				TbFoundType foundType = (TbFoundType)getCommonBo().get(TbFoundType.class, new Long(typeId));
		...
		if(foundStatus!=null&&!"".equals(foundStatus)&&!"0".equals(foundStatus)){
	...
	public String toPage() throws NumberFormatException, Exception{
		...
		if(foundId!=null&&!"".equals(foundId)){
			try{
				bean = (TbFoundation) this.getCommonBo().get(TbFoundation.class, new Long(foundId));
			}catch(Exception e){}
		}
		if(bean==null){
	...
	public String delFound() throws NumberFormatException, Exception{
		...
		String foundId=request.getParameter("foundId");
		if(foundId!=null&&!"".equals(foundId)){
			TbFoundation bean=(TbFoundation) this.getCommonBo().get(TbFoundation.class, new Long(foundId));
			if(bean!=null){
//				String hql = "from TbProtocol a where a.foundId="+bean.getFoundId();
...
//				}
				//先删除协议捐赠详细
				String hql="delete from TbProDetail a where proId in (select proId from TbProtocol b where b.foundId="+bean.getFoundId()+")";
				this.getCommonBo().executeHql(hql);
				//删除协议
				hql="delete from TbProtocol a where a.foundId="+bean.getFoundId();
				this.getCommonBo().executeHql(hql);
				//删除支出
				hql="delete from TbProPay a where a.foundId="+bean.getFoundId();
				this.getCommonBo().executeHql(hql);
				//删除项目
				this.getCommonBo().delete(bean);
	...
	public String toProPage() throws NumberFormatException, Exception{
		...
		if(proId!=null && !"".equals(proId)){
			try{
				bean=(TbProtocol) this.getCommonBo().get(TbProtocol.class, new Long(proId));
			}catch(Exception e){ }
		}
		if(bean==null){
			bean = new TbProtocol();
			...
			if(foundId!=null && !"".equals(foundId)){
				try{
					beanFound=(TbFoundation) this.getCommonBo().get(TbFoundation.class, new Long(foundId));
				}catch(Exception e){ }
			}
			...
			if(syfObject!=null){
				String hql="from TbAcceptCompany a where a.comId in ("+syfObject+")";
			...
			Object obj=null;
			if("1".equals(jzfxz)){
				obj=this.getCommonBo().getHQLUnique("select comName from TbDonationCompany a where a.comId="+jzfObject);
			}else{
				obj=this.getCommonBo().getHQLUnique("select tbAlumni.nameCn from TbDonationor a where a.memId="+jzfObject);
				if(obj==null||"".equals(obj)){
					obj=this.getCommonBo().getHQLUnique("select a.nameCn from TbDonationor a where a.memId="+jzfObject);
				}
			}
	...
	public String saveOrUpdatePro() throws Exception{
		...
		try {
			...
			if(foundId!=null&&!"".equals(foundId)){
				foundation=(TbFoundation) this.getCommonBo().get(TbFoundation.class, new Long(foundId));
				if(foundName!=null){
	...
	public String delPro() throws NumberFormatException, Exception{
		...
		if(point==null||"".equals(point)||!",1,2,".contains(","+point+",")){
			point="1";
		}
		...
		String proId=request.getParameter("proId");
		if(proId!=null&&!"".equals(proId)){
			TbProtocol bean=(TbProtocol) this.getCommonBo().get(TbProtocol.class, new Long(proId));
			if(bean!=null){
...
//				}
				//先删除协议捐赠详细
				String hql="delete from TbProDetail a where proId="+bean.getProId();
				this.getCommonBo().executeHql(hql);
				//再删除协议
				this.getCommonBo().delete(bean);
				request.setAttribute("alert", "删除成功");
...
//			List dorList=this.getCommonBo().findHQL(hql);
			...
			if(type.equals(oldType)){
				...
				//查询已选单位
				if(null!=comIds&&!comIds.equals("")){
					String hql1="from TbDonationCompany a where comId in ("+comIds+")";
				...
				//查询已选个人
				if(null!=comIds&&!comIds.equals("")){
					String hql1="from TbDonationor a where memId in ("+comIds+")";
	...
	public String proDetailList() throws Exception{
		...
		if(bean!=null){
			beanFound = bean.getTbFoundation();
			String hql="from TbProDetail a where a.proId="+bean.getProId()+" order by a.creationTime desc";
			List proDetailList=this.getCommonBo().findHQL(hql);
	...
	public String toProDetailPage() throws NumberFormatException, Exception{
		...
		if(detailId!=null&&!"".equals(detailId)){
			try{
				bean=(TbProDetail) this.getCommonBo().get(TbProDetail.class, new Long(detailId));
			}catch(Exception e){ }
		}
		if(bean==null){
			...
			if(proId!=null && !"".equals(proId)){
				try{
					beanPro=(TbProtocol) this.getCommonBo().get(TbProtocol.class, new Long(proId));
				}catch(Exception e){ }
			}
			if(beanPro!=null){
	...
	public String delProDetail() throws NumberFormatException, Exception{
		...
		String detailId=request.getParameter("detailId");
		if(detailId!=null&&!"".equals(detailId)){
			TbProDetail bean=(TbProDetail) this.getCommonBo().get(TbProDetail.class, new Long(detailId));
			//删除
			this.getCommonBo().delete(bean);
			//
			request.setAttribute("alert", "删除成功");
	...
	public String pay() throws Exception{
		...
		if(payAccount!=null&&!"".equals(payAccount)&&!"0".equals(payAccount)){
			hql+=" and a.payAccount = ?";
			pars.add(payAccount);
		}
		if(jsdwObject!=null&&!"".equals(jsdwObject)&&!"0".equals(jsdwObject)){
	...
	private void getAllTbFields(List tbFieldIdList,Long parentId)throws Exception{
		String hql = "select a from TbFields a where a.parentId="+parentId+" order by a.creationTime";
		List list = this.getCommonBo().findHQL(hql);
		if(list!=null && !list.isEmpty()){
			for(int i=0;i<list.size();i++){
	...
	public String toProPayPage() throws NumberFormatException, Exception{
		...
		if(payId!=null&&!"".equals(payId)){
			TbProPay bean=(TbProPay) this.getCommonBo().get(TbProPay.class, new Long(payId));
			request.setAttribute("bean", bean);
		}
		...
		//查询所有协议下的基金类型
		hql="from TbFields a where a.fieldId in (select parentId from TbFields b where b.fieldId in (select proUse from TbProDetail c where c.proId in (select proId from TbProtocol d where d.foundId="+foundId+")))";
		List fieldList=this.getCommonBo().findHQL(hql);
		...
		//查询协议下实际到账总额
		hql="select sum(toAmount) from TbProDetail a where proId in (select proId from TbProtocol where foundId="+foundId+")";
		Object obj=this.getCommonBo().getHQLUnique(hql);
		obj=obj==null?0:obj;
		
		//所有支出总金额，去除当前支出
		hql="select sum(payAmount) from TbProPay a where a.foundId = "+foundId;
		if(payId!=null&&!"".equals(payId)){
			hql+=" and payId!="+payId;
		}
	...
	public String saveOrUpdateProPay() throws Exception{
		...
		if(payId!=null&&!"".equals(payId)){
			bean=(TbProPay) this.getCommonBo().get(TbProPay.class, new Long(payId));
		}
		if(foundId!=null&&!"".equals(foundId)){
			bean.setFoundId(new Long(foundId));
		}
	...
	public String chooseFound() throws Exception{
		...
		if(foundLevel!=null&&!"".equals(foundLevel)&&!"0".equals(foundLevel)){
			hql+=" and a.foundLevel = ?";
			pars.add(foundLevel);
		}
		if(foundtypeFid!=null&&!"".equals(foundtypeFid)&&!"0".equals(foundtypeFid)){
			hql+=" and a.foundtypeFid = ?";
			pars.add(foundtypeFid);
		}
		if(isKeep!=null&&!"".equals(isKeep)&&!"0".equals(isKeep)){
	...
	public String movePic() throws Exception {
		...
		if(null!=picId&&!"".equals(picId)){
			try {
				bean = (TbProtocolPic)this.getCommonBo().get(TbProtocolPic.class, new Long(picId));
			} catch (Exception e) {	}
		}
		String proId=request.getParameter("proId");
		request.setAttribute("proId", proId);
		...
		String hql = "from TbProtocolPic a where a.proId="+bean.getProId()+" order by a.numOrder";
		List beanList = this.getCommonBo().findHQL(hql);
	...
	public String saveOrUpdatePic() throws Exception {
		...
		if(null!=picItem){
			...
			try{
				...
				String fileDir = com.laungee.proj.common.util.DateUtil.format(dateNow,"yyyyMMdd");
				savePath += fileDir + "/";
	...
	public String moveParam() throws Exception {
		...
		if(null!=paramId&&!"".equals(paramId)){
			try {
				bean = (TbProtocolParam)this.getCommonBo().get(TbProtocolParam.class, new Long(paramId));
			} catch (Exception e) {	}
		}
		String proId=request.getParameter("proId");
		request.setAttribute("proId", proId);
		...
		String hql = "from TbProtocolParam a where a.proId="+bean.getProId()+" order by a.numOrder";
		List beanList = this.getCommonBo().findHQL(hql);
	...
	public String proList() throws Exception{
		...
		if(null!=startDateObj && !"".equals(startDateObj) && null!=endDateObj && !"".equals(endDateObj)){
	...
	public String proListSearch() throws Exception{
		try {
			...
			if(type.equals("1")){//项目情况
				...
				//协议金额
				String hqlXy = "select t.foundId,sum(t.proAmount) from TbProtocol t where t.foundId in (select a.foundId "+hql+") GROUP BY t.foundId,t.tbFoundation.creationTime order by t.tbFoundation.creationTime desc";
				List xyList=this.getCommonBo().findHQL(hqlXy);
				//查询协议下实际到账总额
				String hqlDz="select tp.tbProtocol.foundId,sum(tp.toAmount) from TbProDetail tp where tp.proId in (select p.proId from TbProtocol p where p.foundId in (select a.foundId "+hql+")) GROUP BY tp.tbProtocol.foundId,tp.tbProtocol.tbFoundation.creationTime order by tp.tbProtocol.tbFoundation.creationTime desc";
				List dzList=this.getCommonBo().findHQL(hqlDz);
				//所有支出总金额
				String hqlZc="select t.foundId,sum(t.payAmount) from TbProPay t where t.foundId in (select a.foundId "+hql+") GROUP BY t.foundId,t.tbFoundation.creationTime order by t.tbFoundation.creationTime desc";
				List zcList=this.getCommonBo().findHQL(hqlZc);
				List timeDzList = this.getList();
				List timeZcList = this.getList();
				if(null!=startDate&&!"".equals(startDate)&&null!=endDate&&!"".equals(endDate)){
					hqlDz="select tp.tbProtocol.foundId,sum(tp.toAmount) from TbProDetail tp where tp.toDate>='"+startDate+"' and tp.toDate<='"+endDate+"' and tp.proId in (select p.proId from TbProtocol p where p.foundId in (select a.foundId "+hql+")) GROUP BY tp.tbProtocol.foundId,tp.tbProtocol.tbFoundation.creationTime order by tp.tbProtocol.tbFoundation.creationTime desc";
					timeDzList=this.getCommonBo().findHQL(hqlDz);
					//所有支出总金额
					hqlZc="select t.foundId,sum(t.payAmount) from TbProPay t where t.payDate>='"+startDate+"' and t.payDate<='"+endDate+"' and t.foundId in (select a.foundId "+hql+") GROUP BY t.foundId,t.tbFoundation.creationTime order by t.tbFoundation.creationTime desc";
						...
						if(null!=bean){
							...
							for (int j = 0; j < xyList.size(); j++) {
								...
								if(obj!=null){
									...
									if(foundId!=null && foundId.longValue()==bean.getFoundId().longValue()){
										Object object = obj[1];
							...
							if(null!=startDate&&!"".equals(startDate)&&null!=endDate&&!"".equals(endDate)){
								if((timeDzList!=null && !timeDzList.isEmpty())||(timeZcList!=null && !timeZcList.isEmpty())){
									xyflag = true;
										...
										for (int j = 0; j < timeDzList.size(); j++) {
											...
											if(obj!=null){
												...
												if(foundId!=null && foundId.longValue()==bean.getFoundId().longValue()){
													Object object = obj[1];
													object=object==null?0:object;
													timeDz = new BigDecimal(object.toString());
										...
										for (int j = 0; j < timeZcList.size(); j++) {
											...
											if(obj!=null){
												...
												if(foundId!=null && foundId.longValue()==bean.getFoundId().longValue()){
													Object object = obj[1];
													object=object==null?0:object;
													timeZc = new BigDecimal(object.toString());
							...
							zcBig=zcBig.add(zc);
							xyBig=xyBig.add(xy);
							if(null!=startDate&&!"".equals(startDate)&&null!=endDate&&!"".equals(endDate)&&timeDz==new BigDecimal(0)&&timeZc==new BigDecimal(0)){
								continue;
							}
				...
				String hql="from TbProPay a where 1=1";
				if(null!=startDate&&!"".equals(startDate)&&null!=endDate&&!"".equals(endDate)){
					hql+=" and a.payDate>='"+startDate+"' and a.payDate<='"+endDate+"'";
				}
				if(null!=foundName&&!"".equals(foundName)){
				...
				if(payList!=null && !payList.isEmpty()){
					...
					for (int i = 0; i < payList.size(); i++) {
						...
						if(tbProPay.getTbFields()!=null&&tbProPay.getTbFields().getTbField()!=null){
							cellList.add(tbProPay.getTbFields().getTbField().getFieldName()+"--"+tbProPay.getTbFields().getFieldName());
						}else{
							cellList.add("");
						}
						...
						if(tbProPay.getTbDonationAccout()!=null){
							cellList.add(tbProPay.getTbDonationAccout().getAccountName());
						}else{
							cellList.add("不详");
						}
						...
						if(tbProPay.getJsdwObject()!=null){
							...
							TbAcceptCompany tt = (TbAcceptCompany)this.getCommonBo().get(TbAcceptCompany.class, new Long(jsdwObject));
							if(tt!=null){
								cellList.add(tbProPay.getTbAcceptCompany().getComName());
								cellList.add(tbProPay.getTbAcceptCompany().getRelationName());
							}else{
								cellList.add("不详");	
								cellList.add("不详");	
							}
				...
				if(null!=startDate&&!"".equals(startDate)&&null!=endDate&&!"".equals(endDate)){
					if(type.equals("5")){
						hql += " and a.dealDate>='"+startDate+"' and a.dealDate<='"+endDate+"'";
					}else if(type.equals("3")){
						hql += " and a.toDate>='"+startDate+"' and a.toDate<='"+endDate+"'";
					}else if(type.equals("4")){
						hql += " and a.tbProtocol.dealDate>='"+startDate+"' and a.tbProtocol.dealDate<='"+endDate+"'";
					}
				}
				if(type.equals("5")){
				...
				if(null!=foundName&&!"".equals(foundName)){
					hql += " and a.tbProtocol.tbFoundation.foundName like '%"+foundName+"%'";
					request.setAttribute("foundName", foundName);
				}
				//计算协议总金额
				String sumHql = "select SUM(t.proAmount) from TbProtocol t where t.proId in (select distinct(a.proId) "+hql+")";
				...
				if(null!=proDetailList&&!proDetailList.isEmpty()){
					for (int i = 0; i < proDetailList.size(); i++) {
						TbProDetail tbProDetail = (TbProDetail)proDetailList.get(i);
						List<String> cellList=this.getList();
						cellList.add(tbProDetail.getTbProtocol().getTbFoundation().getFoundName());
						...
						String signStr = "";
						String startTimeStr = tbProDetail.getTbProtocol().getStartTime();
						String endTimeStr = tbProDetail.getTbProtocol().getEndTime();
						...
						String proStatusStr = tbProDetail.getTbProtocol().getProStatus();
						if(null!=proStatusStr && !"".equals(proStatusStr)){
							TbFields fieldBean=(TbFields) this.getCommonBo().get(TbFields.class, new Long(proStatusStr));
							if(fieldBean!=null){
						...
						if(tbProDetail.getTbDonationAccout()!=null){
							cellList.add(tbProDetail.getTbDonationAccout().getAccountName());
						}else{
							cellList.add("");
						}
						if(type.equals("4")){
						...
						if(null!=proUse && !"".equals(proUse)){
							TbFields fieldBean=(TbFields) this.getCommonBo().get(TbFields.class, new Long(proUse));
							if(fieldBean!=null){
								if(fieldBean.getTbField()!=null){
									useStr = fieldBean.getTbField().getFieldName()+"--"+fieldBean.getFieldName();
								}else{
		...
		try {
			...
			if(beanList!=null && !beanList.isEmpty()){
				...
				for (int i = 0; i < titileList.size(); i++) {					
					...
					cell.setCellValue(new HSSFRichTextString((String)titileList.get(i)));
				}
				for(int i=0;i<beanList.size();i++){
					// 单元格
			...
			//保存文件
			String fielCurName = fileName+DateUtil.format(new Date(),"yyyyMMddHHmmss")+".xls";
			...
			for (int i = 0; i < payList.size(); i++) {
				...
				if(tbProPay!=null){
					...
					for(int m=0;m<cellList.size();m++){
						HSSFCell cell = row.createCell((short) (count++));
						cell.setCellStyle(cellStyleSmall);
						cell.setCellValue(new HSSFRichTextString(cellList.get(m)));
					}
				}
