...
public class ZcProjAction extends BaseAction {
	...
	public String execute() throws Exception {
		...
		try {
			bean = (TbZcprojType)getCommonBo().get(TbZcprojType.class, new Long(id));
		} catch (Exception e) {	}
	...
	public String typeJson() throws Exception {
		...
		try{
			// 分类子目录集合
			String hql = "from TbZcprojType a where a.parentId="+id+" order by a.numOrder";
			...
			if(eleList!=null && !eleList.isEmpty()){
				...
				for(int i=0;i<eleList.size();i++){
					...
					String hqlCount = "select count(*) from VwZcprojBase a where a.projType in ("+objIds+")";
					int count = getCommonBo().getHQLNum(hqlCount);
					countList.add(count);
					String hqlCount1 = "select count(*) from VwZcprojBase a where a.projType in ("+objIds+") and a.shelvesFlag='1'";
					int count1 = getCommonBo().getHQLNum(hqlCount1);
					count1List.add(count1);
					String hqlCount2 = "select count(*) from VwZcprojBase a where a.projType in ("+objIds+") and a.shelvesFlag='1' and a.hotFlag='1'";
					int count2 = getCommonBo().getHQLNum(hqlCount2);
					request.setAttribute("count2", count2);
				}
			}
	...
	public String del() throws Exception {
		...
		if(bean!=null){
			//删除项目图片
			String hql="delete from TbZcprojPic a where projId="+bean.getProjId();
			this.getCommonBo().executeHql(hql);
			//删除项目选项
			hql="delete from TbZcprojOption a where projId="+bean.getProjId();
			this.getCommonBo().executeHql(hql);
			//删除项目进度
			hql="delete from TbZcprojProgress a where projId="+bean.getProjId();
			this.getCommonBo().executeHql(hql);
			//删除项目
