...
<body>
	...
	<form id="formId" action="<%=basePath%>commodityOrder.action" method="post" onsubmit="return doSubmit();">
		<table style="width: 100%;" border="0" cellpadding="0" cellspacing="0" class="lyt_search">
			<tr>
				...
				<td width="80">
					<input type="text" name="commName" style="width:80px;" value="<c:out value='${commName}' />" maxlength="100" />
				</td>
				<td class="lyt_search_note" style="width:60px;">
					订单号
				</td>
	...
	<table border="1" cellspacing="0" cellpadding="0" class="lyt_result">
		...
		<tr>
			...
			<td>
				...
				<a href="javascript:;" onclick="MyMsg.confirm('确定要删除该捐赠？','<%=basePath%>commodityOrder!del.action?orderId=${temp.orderId}&${fn:replace(p.pars,'orderId','radom')}');">删除</a>
			</td>
		</tr>
		</c:forEach>
	</table>
