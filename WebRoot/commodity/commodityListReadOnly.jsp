...
<script type="text/javascript">
	...
	function goEdit(id){
		top.uploadTab('商品【'+id+'】详情','商品【'+id+'】详情','<%=basePath%>commodityReadOnly!operation.action?commId='+id);
	}
	function goTupianMgt(id){
		top.uploadTab('商品【'+id+'】详情','商品【'+id+'】详情','<%=basePath%>commodityReadOnly!info.action?commId='+id+'&num=1');
	}
	function goCanshuMgt(id){
		top.uploadTab('商品【'+id+'】详情','商品【'+id+'】详情','<%=basePath%>commodityReadOnly!info.action?commId='+id+'&num=2');
	}
	function goXinghaoMgt(id){
		top.uploadTab('商品【'+id+'】详情','商品【'+id+'】详情','<%=basePath%>commodityReadOnly!info.action?commId='+id+'&num=3');
	}
	function goDingdanMgt(id){
		top.uploadTab('商品【'+id+'】详情','商品【'+id+'】详情','<%=basePath%>commodityReadOnly!info.action?commId='+id+'&num=4');
	}
</script>
<body>
	<div class="page">
	<%@ include file="../common/navigation.jsp"%>
		<form id="formId" action="<%=basePath%>commodityReadOnly!list.action" method="get" onsubmit="return doSubmit()">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="lyt_search">
				...
				<td align="right" class="lyt_search_opt">
					<div class="lyt_button">
						<input type="submit" value="查询" style="margin-right:20px"/> 
					</div>
				</td>
			</tr>
			</table>
		...
		<table class="lyt_result" style="width:100%">
			<tr>
				<th width="40px">序号</th>
			...
			<c:forEach items="${beanList }" var="bean" varStatus="status">
				<tr>
					...
					<td>
						<a onclick="goEdit('${bean.commId}');" href="javascript:;">详情</a>
		...
		<my:page action="commodityReadOnly!list.action"></my:page>	
	</div>
</body>
</html>