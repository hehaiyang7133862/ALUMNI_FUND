...
<html>
	<head>
		...
		function checkForm(form){
			...
			if($("#shippingCurfee").val() != "" && !/^\d+(.\d{1,2})?$/.test($("#shippingCurfee").val())){
				MyMsg.alertFn('实用运费（最多两位小数的正整数）！',function(){
					$("#shippingCurfee").focus();
				});
				return false;
			}
	...
	<body>
		<div class="float">
		<form id="formId" name="formId" action="<%=basePath%>commodity!orderMarkUpdate.action" method="post" onsubmit="return checkForm(this);">
			...
			<table class="lyt_view">
				...
				<tr>
					...
					<td>
						<input style="width:150px" type="text" name="shippingCurfee" id="shippingCurfee" value="<c:out value='${bean.shippingCurfee}'/>" maxlength="50" />
						元
					</td>
				</tr>
				<tr>
			...
			<my:timeView idField="orderId" table="TbCommodityOrder" value="${bean.orderId}"></my:timeView>
			<div class="lyt_submit" style="width: 724px; text-align: right;">
				<input type="button" value="订单详情" style="letter-spacing:0px;" onclick="parent.goView('${bean.orderId}');" />
