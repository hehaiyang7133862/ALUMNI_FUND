...
<body>
	...
	<form id="formId" action="<%=basePath%>commodity!info.action"
			...
			<tr>
				...
				<td align="right" class="lyt_search_opt">
					<div class="lyt_button">
						<input value="刷新" type="submit" /> <input value="新增"
							type="button"
							onclick="MyFormWin.showMyWin('新增参数','<%=basePath%>commodity!operationParam.action?commId=${bean.commId}',580,310);return false;" />
					</div>
				</td>
			</tr>
		</table>
	...
	<table border="1" cellspacing="0" cellpadding="0" class="lyt_result">
		...
		<c:forEach items="${beanList}" var="temp" varStatus="status">
			<tr>
				...
				<td>${temp.memo}</td>
				<td><a
					onclick="MyFormWin.showMyWin('修改参数','<%=basePath%>commodity!operationParam.action?paramId=${temp.paramId}&commId=${bean.commId}',580,310);return false;"
					href="javascript:;">编辑</a> <a
					onclick="MyMsg.confirm('您确定要删除吗？','<%=basePath%>commodity!deleteParam.action?paramId=${temp.paramId}&commId=${bean.commId}&num=${num}');"
