...
<body>
	<div class="float">
		<form id="formId"
			action="<%=basePath%>commodity!saveOrUpdateParam.action"
			method="post" onSubmit="return checkForm(this)">
			<input name="commId" type="hidden" value="${bean.commId}" /> <input
				name="paramId" type="hidden" value="${bean.paramId}" />
			<table border="0" cellspacing="0" cellpadding="0" class="lyt_view">
				<tr>
					<td class="lyt_view_note" style="width:80px;">参数名称 <span>*</span>
					</td>
					<td><input type="text" style="width:100px;" id="paramName"
						name="paramName" value="<c:out value='${bean.paramName}'/>" />
					</td>
					<td class="lyt_view_note" style="width:80px;">是否对外显示<span>*</span>
					</td>
					<td width="80"><select name="paramPublish" style="width:100px;">
							<option value="1"
								<c:if test="${bean.paramPublish=='1'}">selected</c:if>>对外显示</option>
							<option value="0"
								<c:if test="${bean.paramPublish=='0'}">selected</c:if>>不对外显示</option>
					</select>
				</tr>
				<tr>
					<td class="lyt_view_note">参数值</td>
					<td colspan="3"><input type="text" style="width:400px;" name="paramValue"
						id="paramValue" value="<c:out value='${bean.paramValue}'/>"
						maxlength="200" />
					</td>
				</tr>
				<tr>
					<td class="lyt_view_note">参数排序</td>
					<td colspan="3"><input type="text" style="width:400px;" name="numOrder"
						id="numOrder" value="<c:out value='${bean.numOrder}'/>"
						maxlength="50" />
					</td>
				</tr>
				<tr>
			...
			<my:timeView idField="paramId" table="TbCommodityParam"
				value="${bean.paramId}"></my:timeView>
