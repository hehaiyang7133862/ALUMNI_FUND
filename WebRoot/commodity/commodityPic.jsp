				...
				<td align="right" class="lyt_search_opt">
					<div class="lyt_button">
						<input value="刷新" type="submit" /> <input value="新增"
							type="button"
							onclick="MyFormWin.showMyWin('新增图片','<%=basePath%>commodity!operationPic.action?commId=${bean.commId}',580,310);return false;" />
					</div></td>
			</tr>
		</table>
	</form>
	<table border="1" cellspacing="0" cellpadding="0" class="lyt_result">
		...
		<c:forEach items="${beanList}" var="pic" varStatus="status">
			<tr>
				...
				<td>${pic.memo}</td>
				<td><a
					onclick="MyFormWin.showMyWin('修改图片','<%=basePath%>commodity!operationPic.action?picId=${pic.picId}&commId=${bean.commId}',580,380);return false;"
					href="javascript:;">编辑</a> <a
					onclick="MyMsg.confirm('您确定要删除吗？','<%=basePath%>commodity!deletePic.action?picId=${pic.picId}&commId=${bean.commId}&num=${num}');"
