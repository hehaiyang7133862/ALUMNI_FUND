...
FormEditWin = function(){
	...
	return {
		...
		showAddWin : function(parentNode) {
			...
			var editpage = spath
					+ "foundType!add.action?parentId="+ parentNode.id;
		...
		showEditWin : function(node) {
			// 显示目录编辑窗口
			var editpage = spath
					+ "foundType!edit.action?id=" + node.id;
...
Ext.onReady(function(){
	...
	if(typeof(FoundTypeManager)=="undefined"){
		Ext.Msg.alert("警告提示","请先设置DWR");
	}else{
		NavTree.setMgr(FoundTypeManager);
		NavTree.init();
		NavTree.show();
	}
});