...
<html>
	<head>
		...
		function goNew(){
			top.uploadTab('项目管理 > 新建项目','新建项目','<%=basePath%>found!toPage.action?typeId=${typeId}&type=${type}');
		}
		function goEdit(id){
			top.uploadTab('项目【'+id+'】管理','项目【'+id+'】管理','<%=basePath%>found!toPage.action?foundId='+id);
		}
		function goXieyiMgt(id){
			top.uploadTab('项目【'+id+'】管理','项目【'+id+'】管理','found!procl.action?foundId='+id+'&num=1');
		}
		function goZhichuMgt(id){
			top.uploadTab('项目【'+id+'】管理','项目【'+id+'】管理','found!pay.action?foundId='+id+'&num=2');
		}
		function goZhixingMgt(id){
			top.uploadTab('项目【'+id+'】管理','项目【'+id+'】管理','found!doc.action?foundId='+id+'&num=3');
		}
		function goDelete(id){
			MyMsg.confirmFn('确定要删除该项目？',function(){
				location='<%=basePath%>found!delFound.action?foundId='+id+'&${fn:replace(p.pars,'foundId','radom')}';
			});
		}
		</script>
	</head>
	<body>
		<div class="page">
			...
			<form id="formId" action="<%=basePath%>found!list.action" method="get" onsubmit="return doSubmit()">
			...
			<table width="100%" border="0" cellspacing="0" class="lyt_result">
				...
				<tr>
					...
					<td>
						<a href="javascript:;" onclick="goEdit('${temp.foundId}');">编辑</a>
						...
						<a href="javascript:;" onclick="goDelete('${temp.foundId}');">删除</a>
					</td>
				</tr>
				</c:forEach>
			</table>
			<my:page action="found!list.action"></my:page>
		</div>
	</body>
</html>