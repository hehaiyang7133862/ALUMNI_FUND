...
<head>
	...
	<script type="text/javascript">
		function goNew(){
			top.uploadTab('协议管理 > 新建协议','新建协议','<%=basePath%>found!toProPage.action?point=2&foundId=${foundId}');
		}
		function goEdit(id){
			top.uploadTab('协议【'+id+'】管理','协议【'+id+'】管理','<%=basePath%>found!toProPage.action?point=2&proId='+id+'&foundId=${foundId}');
		}
		function goDelete(id){
			MyMsg.confirmFn('确定要删除该项目协议？',function(){
				location='<%=basePath%>found!delPro.action?point=2&proId='+id+'&foundId=${foundId}&${fn:replace(p.pars,'foundId','radom')}';
			});
		}
	</script>
</head>
	<body>
		<div class="page">
			...
			<form id="adviseForm" action="<%=basePath%>found!procl.action" method="post" onsubmit="">
