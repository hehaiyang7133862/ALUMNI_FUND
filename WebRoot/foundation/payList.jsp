...
<head>
	...
	<body>
		<div class="page">
			...
			<form id="formId" action="<%=basePath%>found!pay.action?sub=${sub}" method="get" onsubmit="return doSubmit()">
					...
					<tr>
						...
						<td align="right" class="lyt_search_opt" colspan="4">
							<div class="lyt_button">
								...
								<input name="" value="新增" type="button" onClick="MyFormWin.showMyWin('项目支出管理详细','<%=basePath%>found!toProPayPage.action?foundId=${foundId}',650,398);return false;" />
							</div>
						</td>
					</tr>
					<tr>
			...
			<table width="100%" border="0" cellspacing="0" class="lyt_result">
				<tr>
					...
					<th>基金用途</th>
					<th>
						<my:order action="found!pay.action" title="支出金额（元）" name="a.payAmount" />
					</th>
					<th>支出账户</th>
					<th>
						<my:order action="found!pay.action" title="支出日期" name="a.payDate" />
					</th>
					<th>接受单位</th>
					<th width="60">操作</th>
				</tr>
				<c:forEach items="${payList}" var="temp" varStatus="status">
					<tr>
						...
						<td>
							...
							<a href="javascript:;" onclick="MyMsg.confirm('是否确定删除','<%=basePath%>found!delProPay.action?payId=${temp.payId}&${fn:replace(p.pars,'payId','radom')}');">删除</a>
						</td>
					</tr>
				</c:forEach>
			</table>
			<my:page action="found!pay.action"></my:page>
		</div>
	</body>
</html>