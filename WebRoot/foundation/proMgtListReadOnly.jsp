...
<head>
	...
	<script type="text/javascript">
		...
		function goEdit(id,found){
			top.uploadTab('协议【'+id+'】详情','协议【'+id+'】详情','<%=basePath%>foundReadOnly!toProPage.action?proId='+id+'&foundId='+found);
		}
		function goMingxiMgt(id){
			top.uploadTab('协议【'+id+'】详情','协议【'+id+'】详情','<%=basePath%>foundReadOnly!proDetailList.action?point=2&proId='+id+'&foundId=${foundId}&num=1');
		}
	</script>
</head>
	<body>
		<div class="page">
			...
			<form id="formId" action="<%=basePath%>foundReadOnly!proclMgt.action" method="get" onsubmit="return doSubmit()">
			...
			<table style="width: 100%;" border="0" cellspacing="0" class="lyt_result">
				<tr>
					<th width="40">序号</th>
					<th>
						<my:order action="foundReadOnly!proclMgt.action" title="项目名称" name="a.tbFoundation.foundName" />
					</th>
					<th width="70">
						<my:order action="foundReadOnly!proclMgt.action" title="协议编号" name="a.proCode" />
					</th>
					<th>
						<my:order action="foundReadOnly!proclMgt.action" title="协议名称" name="a.proName" />
					</th>
					<th width="105">
						<my:order action="foundReadOnly!proclMgt.action" title="协议金额（元）" name="a.proAmount" />
					</th>
					<!-- <th>实际到账（元）</th> -->
					<th width="100">捐赠方</th>
					<th width="100">受赠方</th>
				...
				<c:forEach items="${proList}" var="temp" varStatus="status">
					<tr>
						...
						<td>
							<a href="javascript:;" onclick="goEdit('${temp.proId}','${temp.foundId}');">详情</a>
							<a href="javascript:;" onclick="goMingxiMgt('${temp.proId}');">明细</a>
						</td>
					</tr>
				</c:forEach>
			</table>
			<my:page action="foundReadOnly!proclMgt.action"></my:page>
		</div>
	</body>
</html>