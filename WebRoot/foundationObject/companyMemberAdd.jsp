...
$(document).ready(function(){
	$("input[type='button'][class='btn_save']").click(function(){
		...
		if(checkForm(form[0])){
			...
			$.ajax({
				type:"POST",
				url:"<%=basePath%>donate!saveOrUpdateMember.action",
				data:form.serialize(),
				dataType:"json",
	...
	<body>
		...
		<form id="formId" name="formId" method="post">
			...
			<table class="lyt_view">
				...
				<tr>
					...
					<td style="width: 160px;">
						<c:choose>
						<c:when test="${not empty bean.tbWork}">
							<input style="width: 155px;" type="text" name="dutyContent" id="dutyContent" value='<c:out value="${bean.tbWork.dutyContent}" />' />
						</c:when>
						<c:otherwise>
							<input style="width: 155px;" type="text" name="dutyContent" id="dutyContent" value='<c:out value="${bean.dutyContent}" />' />
						</c:otherwise>
						</c:choose>
					</td>
					<td class="lyt_view_note" style="width: 80px;">
			...
			<my:timeView idField="memId" table="TbDonationCompanyMember" value="${bean.memId}"></my:timeView>
