	...
	return {
		...
		showAddWin : function(parentNode) {
			...
			var editpage = basePath
					+ "menuPreAdd.action?parentId="+ parentNode.id;
			var window = this.createWin("winleafnew", "新建子级角色", editpage, function() {
				parentNode.reload();
			});
			window.show();
		},
		...
		init : function(){
			...
			if(!loader){
				loader = new Ext.tree.TreeLoader({
					url : basePath + '/buttonList.action'
				});
