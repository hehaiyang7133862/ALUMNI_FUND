...
FormEditWin = function(){
	...
	return {
		...
		showAddWin : function(parentNode) {
			...
			var editpage = spath
					+ "fieldAdd!pre.action?parentId="+ parentNode.id;
		...
		showEditWin : function(node) {
			// 显示目录编辑窗口
			var editpage = spath
					+ "fieldEdit!pre.action?id=" + node.id;
...
Ext.onReady(function(){
	...
	if(typeof(FieldManager)=="undefined"){
		Ext.Msg.alert("警告提示","请先设置DWR，并实例化FieldManager");
	}else{
		NavTree.setMgr(FieldManager);
		NavTree.init();
		NavTree.show();
	}
});