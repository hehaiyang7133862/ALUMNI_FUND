...
FormEditWin = function(){
	...
	return {
		...
		showViewWin   : function(node){
			document.getElementById("iFrameId").src="roleIFrame.action?id="+node.id+"&grandId="+sid;
		},
		// 添加
		showAddWin : function(parentNode) {
			...
			var editpage = basePath
					+ "roleAdd!pre.action?parentId="+ parentNode.id;
			var window = this.createWin("winleafnew", "菜单添加窗口-"+sname, editpage, function() {
				parentNode.reload();
			});
			window.show();
		},
		...
		showEditWin : function(node) {
			// 显示目录编辑窗口
			var editpage = basePath
					+ "roleEdit!pre.action?id=" + node.id;
			var window = this.createWin("win" + node.id, "菜单编辑窗口-"+node.text, editpage, function() {
...
Ext.onReady(function(){
	...
	if(typeof(RoleManager)=="undefined"){
		Ext.Msg.alert("警告提示","请先设置DWR，并实例化RoleManager");
	}else{
		NavTree.setMgr(RoleManager);
		NavTree.init();
		NavTree.show();
	}
});