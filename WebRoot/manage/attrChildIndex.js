	...
	return {
		...
		showAddWin : function(parentNode) {
			...
			var editpage = basePath
					+ "attrChildPreAdd.action?parentId="+parentNode.id;
			var window = this.createWin("winAdd", "新建"+sname, editpage, function() {
				parentNode.reload();
			});
			window.show();
		},
		...
		showEditWin : function(node) {
			// 显示目录编辑窗口
			var editpage = basePath
					+ "attrChildPreEdit.action?id=" + node.id;
			var window = this.createWin("winEdit" + node.id, node.text, editpage, function() {
