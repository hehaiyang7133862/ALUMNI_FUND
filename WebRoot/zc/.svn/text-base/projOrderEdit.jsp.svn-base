...
<html>
	...
	<body>
		<div class="page">
			...
			<form id="formId" name="formId" action="<%=basePath%>zcProjEdit!doAddOrEdit.action" method="post" onsubmit="return checkForm(this);">
				...
				<table class="lyt_view">
					...
					<tr>
						...
						<td colspan="3">
							<c:out value="${beanZcproj.projName}" />
						</td>
					</tr>
					<tr>
						<td class="lyt_view_note" style="width:70px;">
							...
							<input type="hidden" id="optionDefineName" value=""/>
						</td>
						<td style="width:380px;">
							<select id="option" name="option" style="width:261px;" onchange="optionChange();">
								<c:if test="${not empty bean.optionId || not empty bean.optionName}">
								<optgroup label="当前">
									<option value="${bean.optionId}" valueName="<c:out value="${bean.optionName}"/>" selected><c:out value="${bean.optionName}"/><c:if test="${not empty bean.optionId}">（${bean.optionId}）</c:if></option>
								</optgroup>
								</c:if>
								<optgroup label="系统">
									<c:if test="${beanZcproj.optionOther=='1'}">
									<option value="" valueName="<c:out value="${beanZcproj.optionOtherName}"/>"><c:out value="${beanZcproj.optionOtherName}"/>（<fmt:formatNumber value='${beanZcproj.minAmount}' pattern='0.##' type='number'/>元起）</option>
									</c:if>
									<c:forEach items="${optionList}" var="opttemp">
									<option value="${opttemp.optionId}" valueName="<c:out value="${opttemp.optionName}"/>"><c:out value="${opttemp.optionName}"/>（${opttemp.optionId} ▪ <fmt:formatNumber value='${opttemp.optionAmount}' pattern='0.##' type='number'/>元/份）</option>
									</c:forEach>
								</optgroup>
							</select>
					...
					<tr>
						...
						<td>
							<input type="text" id="amountView" name="amountView" class="txt" style="width:261px;" value="<fmt:formatNumber value='${bean.orderAmountView}' pattern='0.##' type='number'/>" maxlength="50" autocomplete="OFF"/>
		...
		<script type="text/javascript">
			...
			function optionChange(){
				if($('#option').val()==''){
					$('#optionCountSpan').hide();
					$('#optionCount').val("");
					$('#amount').attr("readonly",false).css('background-color','#fff').val("").focus();
				}else{
					$('#optionCountSpan').show();
					$('#optionCount').val("1").focus();
					$('#amount').attr("readonly",true).css('background-color','#eee').val($('#option option:selected').attr('singleAmount'));
				}
			}
			function optionCountChange(){
				var single = parseFloat($('#option option:selected').attr('singleAmount'));
				var count = parseInt($('#optionCount').val());
				if(!isNaN(single) && !isNaN(count)){
					$('#amount').val(single*count);
				}
			}*/
			function optionDefine(){
				Ext.Msg.buttonText.ok='确定';
			    Ext.Msg.buttonText.cancel="取消";
				Ext.Msg.prompt("自定义选项", "请输入自定义选项名称（50字以内）：", function(btn, text){
				    if(btn=='ok'){
				    	var value = text.replace(/(^\s*)|(\s*$)/g, "");
				        if(value.length==0){
				    		$("#optionDefineName").val(value);
			            	MyMsg.alertFn("请输入自定义选项名称!",function(){optionDefine();});
			            }else if(value.length>50){
				    		$("#optionDefineName").val(value);
			            	MyMsg.alertFn("自定义选项名称不能超过50字!",function(){optionDefine();});
			            }else{
				    		$("#optionDefineName").val('');
			            	if($('#option > optgroup[label="自定义"]').length==0){
			            		$('#option').append('<optgroup label="自定义"></optgroup>')
			            	}
				        	$('#option > optgroup[label="自定义"]').append('<option value="" valueName="'+value+'" selected>'+value+'</option>')
				        	$('#optionName').val(value);
				        }
				    }
				}, this, false, $("#optionDefineName").val());
			}
			function optionChange(){
				$('#optionName').val($('#option option:selected').attr('valueName'));
			}
			...
			function checkForm(){
				...
				if($("#optionCount").val() != '' && !/^\d+$/.test($("#optionCount").val())){
					MyMsg.alertFn('请输入捐赠份数（正整数）！',function(){
						$("#optionCount").focus();
					});
					return false;
				}
				if($('#amountType').val()==''){
					MyMsg.alertFn('请输入捐赠币种！',function(){
						$('#amountType').focus();
					});
					return false;
				}
