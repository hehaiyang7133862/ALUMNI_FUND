...
<html>
	<head>
		...
		function goMark(id){
			MyFormWin.close();
			MyFormWin.showMyWin('捐赠标记','<%=basePath%>zcProjOrder!mark.action?orderId='+id,800,400);
		}
		function goView(id){
			MyFormWin.close();
			MyFormWin.showMyWin('捐赠详情','<%=basePath%>zcProjOrder!view.action?orderId='+id,580,310);
		}
		function goDel(id){
			MyMsg.confirm('确定要删除该捐赠？','<%=basePath%>zcProjOrder!del.action?orderId='+id+'&${fn:replace(p.pars,'orderId','radom')}');
		}
		// 导出全部捐赠信息
		function exportAll(){
			MyMask.showHtml("正在导出捐赠信息,请稍后....");
			$.ajax({ 
			    url:"<%=basePath%>zcProjOrder!doExport.action", 
			    type:"post",
			    dataType:'text',
			    cache:false,
			    ...
			    success:function(data){
					...
					if(json.result=="success"){
						setTimeout(function(){
							location.href = "fileDown!downImp.action?name=众筹项目捐赠信息.xls&path=" + json.path;
						},0);
					}else{
		...
		$(document).ready(function(){
			windowResizeFn.add(function(){
				...
				$('#resultDiv').height(document.body.clientHeight-180-$('.lyt_nav').height());
				if($('.lyt_result').width()>$('#resultDiv').width()){
					$('.lyt_result').css('width','2300px');
				}else{
	...
	<body>
		<div class="page">
			...
			<form id="formId" action="<%=basePath%>zcProjOrder.action" method="get" onsubmit="return doSubmit()">
				<table style="width: 100%;" border="0" cellpadding="0" cellspacing="0" class="lyt_search">
					...
					<tr>
						...
						<td>
							<input type="text" name="amountType" style="width:80px;margin-top:4px;" value="<c:out value='${amountType}' />" />
						</td>
						<td class="lyt_search_note">
							捐赠人类型
						</td>
			...
			<table border="1" cellspacing="0" cellpadding="0" class="lyt_result" style="width:2300px;">
				<tr>
					<th width="40">序号</th>
					<th width="80">
						<my:order action="zcProjOrder.action" title="捐赠单号" name="a.orderNum" />
					</th>
					<th width="135">
						<my:order action="zcProjOrder.action" title="项目名称" name="a.projName" />
					</th>
					<th width="80">
						<my:order action="zcProjOrder.action" title="项目选项" name="a.optionName" />
					</th>
					<th width="40">
						<my:order action="zcProjOrder.action" title="份数" name="a.optionCount" />
					</th>
					<th width="60">
						<my:order action="zcProjOrder.action" title="捐赠币种" name="a.orderAmountType" />
					</th>
					<th width="60">
						<my:order action="zcProjOrder.action" title="捐赠币种总金额" name="a.orderAmountView" />
					</th>
					<th width="60">
						<my:order action="zcProjOrder.action" title="实算总金额（元）" name="a.orderAmount" />
					</th>
					<th width="100">
						<my:order action="zcProjOrder.action" title="捐赠方式" name="a.orderType" />
					</th>
					<th width="45">捐赠状态</th>
					<th width="85">
						<my:order action="zcProjOrder.action" title="提交时间" name="a.orderTime" />
					</th>
					<th width="85">
						<my:order action="zcProjOrder.action" title="付款时间" name="a.orderOkTime" />
					</th>
					<th width="30">
						<my:order action="zcProjOrder.action" title="捐赠人类型" name="a.personType" />
					</th>
					<th width="30">
						<my:order action="zcProjOrder.action" title="捐赠人数" name="a.personCount" />
					</th>
					<th width="80">操作</th>
					<th width="30">需要证书</th>
					<th width="30">需要发票</th>
				...
				<tr>
					...
					<c:choose>
					<c:when test="${not empty temp.projId && not empty temp.tbZcproj}">
					<td style="color:blue;" title="<c:out value="${temp.projName}（${temp.projId}）"/>">
						<c:out value="${temp.projName}" />
					</td>
					</c:when>
					<c:when test="${not empty temp.projId && empty temp.tbZcproj}">
					<td style="color:#555;" title="<c:out value="${temp.projName}（${temp.projId}）"/>（项目已移除）">
						<c:out value="${temp.projName}" />
					</td>
					</c:when>
					<c:otherwise>
			...
			<my:page action="zcProjOrder.action"></my:page>
			<div class="lyt_submit" style="margin-top:15px;padding:10px 0px 0px 0px;color:blue;font-weight:bold;text-align:center;">
