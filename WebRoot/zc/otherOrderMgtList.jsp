...
<html>
	<head>
		...
		function goMark(id){
			MyFormWin.close();
			MyFormWin.showMyWin('捐赠标记','<%=basePath%>zcOtherOrder!mark.action?orderId='+id,800,400);
		}
		function goView(id){
			MyFormWin.close();
			MyFormWin.showMyWin('捐赠详情','<%=basePath%>zcOtherOrder!view.action?orderId='+id,580,310);
		}
		function goEdit(id){
			MyFormWin.close();
			MyFormWin.showMyWin('捐赠编辑','<%=basePath%>zcOtherOrder!toAddOrEdit.action?orderId='+id,850,500);
		}
		function goDel(id){
			MyMsg.confirm('确定要删除该捐赠？','<%=basePath%>zcOtherOrder!del.action?orderId='+id+'&${fn:replace(p.pars,'orderId','radom')}');
		}
		// 下载模板
		function downModel(){
		...
		function importModel(){
			parent.addTab("其他捐赠信息导入","其他捐赠信息导入","zcOtherOrder!toImport.action");
		}
		// 导出全部捐赠信息
		function exportAll(){
			MyMask.showHtml("正在导出其他捐赠信息,请稍后....");
			$.ajax({ 
			    url:"<%=basePath%>zcOtherOrder!doExport.action", 
			    type:"post",
			    dataType:'text',
			    cache:false,
			    data:$('#formId').serialize(),
			    success:function(data){
					...
					if(json.result=="success"){
						setTimeout(function(){
							location.href = "fileDown!downImp.action?name=其他捐赠信息.xls&path=" + json.path;
						},0);
					}else{
						MyMsg.alert("其他捐赠信息导出失败！");
					}
					setTimeout(function(){
						MyMask.hide();
					},0);
	...
	<body>
		<div class="page">
			...
			<form id="formId" action="<%=basePath%>zcOtherOrder.action" method="get" onsubmit="return doSubmit()">
					...
					<tr>
						...
						<td align="right" class="lyt_search_opt">
							<div class="lyt_button">
								...
								<input value="新增" type="button" onclick="MyFormWin.showMyWin('捐赠新增','<%=basePath%>zcOtherOrder!toAddOrEdit.action',850,500);"/>
			...
			<table border="1" cellspacing="0" cellpadding="0" class="lyt_result" style="width:2140px;">
				<tr>
					<th width="40">序号</th>
					<th width="80"><my:order action="zcOtherOrder.action" title="捐赠单号" name="a.orderNum" /></th>
					<th width="120"><my:order action="zcOtherOrder.action" title="项目名称" name="a.projName" /></th>
					<th width="60"><my:order action="zcOtherOrder.action" title="捐赠币种" name="a.orderAmountType" /></th>
					<th width="60"><my:order action="zcOtherOrder.action" title="捐赠币种总金额" name="a.orderAmountView" /></th>
					<th width="60"><my:order action="zcOtherOrder.action" title="实算总金额（元）" name="a.orderAmount" /></th>
					<th width="100"><my:order action="zcOtherOrder.action" title="捐赠方式" name="a.orderType" /></th>
					<th width="45">捐赠状态</th>
					<th width="85"><my:order action="zcOtherOrder.action" title="提交时间" name="a.orderTime" /></th>
					<th width="85"><my:order action="zcOtherOrder.action" title="付款时间" name="a.orderOkTime" /></th>
					<th width="30"><my:order action="zcOtherOrder.action" title="捐赠人类型" name="a.personType" /></th>
					<th width="30"><my:order action="zcOtherOrder.action" title="捐赠人数" name="a.personCount" /></th>
					<th width="100">操作</th>
					<th width="30">需要证书</th>
					<th width="30">需要发票</th>
					<th width="30">匿名捐赠</th>
			...
			<my:page action="zcOtherOrder.action"></my:page>
