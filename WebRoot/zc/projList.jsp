...
<html>
	<head>
		...
		function goNew(){
			top.uploadTab('众筹管理 > 新建众筹','新建众筹','<%=basePath%>zcProjEdit.action?typeId=${typeId}');
		}
		function goEdit(id){
			top.uploadTab('众筹【'+id+'】管理','众筹【'+id+'】管理','<%=basePath%>zcProjEdit.action?projId='+id);
		}
		function goTupianMgt(id){
			top.uploadTab('众筹【'+id+'】管理','众筹【'+id+'】管理','<%=basePath%>zcProjEdit!picList.action?projId='+id+'&num=1');
		}
		function goXuanxiangMgt(id){
			top.uploadTab('众筹【'+id+'】管理','众筹【'+id+'】管理','<%=basePath%>zcProjEdit!optionList.action?projId='+id+'&num=2');
		}
		function goJinzhanMgt(id){
			top.uploadTab('众筹【'+id+'】管理','众筹【'+id+'】管理','<%=basePath%>zcProjEdit!gressList.action?projId='+id+'&num=3');
		}
		function goJuanzengMgt(id){
			top.uploadTab('众筹【'+id+'】管理','众筹【'+id+'】管理','<%=basePath%>zcProjEdit!orderList.action?projId='+id+'&num=4');
		}
		function goDelete(id){
			MyMsg.confirmFn('确定要删除该众筹项目？',function(){
				location.href='<%=basePath%>zcProj!del.action?projId='+id+'&${fn:replace(p.pars,'projId','radom')}';
			});
		}
	...
	<body>
		<div class="page">
			...
			<form id="formId" action="<%=basePath%>zcProj!list.action" method="get" onsubmit="return doSubmit()">
			...
			<table border="0" cellspacing="0" class="lyt_result" style="width:1800px;">
				<tr>
					<th width="40">序号</th>
					<th>
						<my:order action="zcProj!list.action" title="项目名称" name="a.projName" />
					</th>
					<th width="60">操作</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="显示发布时间" name="a.shelvesTime" />
					</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="众筹目标" name="a.targetAmout" />
					</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="众筹开始时间" name="a.begTime" />
					</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="众筹截止时间" name="a.endTime" />
					</th>
					<th width="30">上架</th>
					<th width="30">热门</th>
					<th width="65">
						<my:order action="zcProj!list.action" title="热门排序" name="a.hotOrder" />
					</th>
					<th width="65">目标完成后继续捐赠</th>
					<th width="65">项目结束后归为经典</th>
					<th width="60">众筹状态</th>
					<th width="60">
						<my:order action="zcProj!list.action" title="结束剩余天数" name="a.endLastDay" />
					</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="筹得" name="a.zcedAmout" />
					</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="支持笔数" name="a.zcedCount" />
					</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="支持人数" name="a.zcedPersonCount" />
					</th>
					<th width="100">
						<my:order action="zcProj!list.action" title="目标完成度" name="a.zcedPercent" />
					</th>
					<th width="60">操作</th>
				</tr>
				...
				<tr>
					...
					<td title="<c:out value="${temp.projName}"/>&#10;编码：<c:out value="${temp.projCode}"/>&#10;标签：<c:out value="${temp.searchKey}"/>">
						...
						<p style="line-height:20px;">标签：<c:forEach items="${fn:split(temp.searchKey,',')}" var="searchKeyTemp" varStatus="searchKeyStatus"><a href="<%=basePath%>zcProj!list.action?searchKey=<c:out value="${searchKeyTemp}"/>&<c:out value="${fn:replace(p.pars,'searchKey','radom')}"/>"><c:out value="${searchKeyTemp}"/></a></c:forEach></p>
						</c:if>
					</td>
					<td title="操作">
						<a href="javascript:;" onclick="goEdit('${temp.projId}');">编辑</a>
						<!-- 
						<a href="javascript:;" onclick="goTupianMgt('${temp.projId}');">图片</a>
						<a href="javascript:;" onclick="goXuanxiangMgt('${temp.projId}');">选项</a>
						<a href="javascript:;" onclick="goJinzhanMgt('${temp.projId}');">进展</a>
						<a href="javascript:;" onclick="goJuanzengMgt('${temp.projId}');">捐赠</a>
						 -->
						<a href="javascript:;" onclick="goDelete('${temp.projId}');">删除</a>
					</td>
			...
			<my:page action="zcProj!list.action"></my:page>
		</div>
	</body>
</html>