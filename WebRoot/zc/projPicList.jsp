...
<html>
	...
	<body>
		<div class="page">
			...
			<form id="formId" action="<%=basePath%>zcProjEdit!picList.action" method="get" onsubmit="return doSubmit()">
					...
					<tr>
						...
						<td align="right" class="lyt_search_opt">
							<div class="lyt_button">
								...
								<input value="新增" type="button" onclick="MyFormWin.showMyWin('新增图片','<%=basePath%>zcProjEdit!picEdit.action?projId=${bean.projId}',580,310);return false;" />
							</div>
						</td>
					</tr>
				</table>
			...
			<table border="1" cellspacing="0" cellpadding="0" class="lyt_result">
				...
				<tr>
					...
					<td>
						<a href="javascript:;" onclick="MyFormWin.showMyWin('编辑图片','<%=basePath%>zcProjEdit!picEdit.action?projId=${bean.projId}&picId=${temp.picId}',580,310);return false;">编辑</a>
						<a href="javascript:;" onclick="MyMsg.confirm('确定要删除该图片？','<%=basePath%>zcProjEdit!picDel.action?projId=${bean.projId}&picId=${temp.picId}&num=${num}');">删除</a>
					</td>
				</tr>
				</c:forEach>
			</table>
